      ...
      <el-table-column v-if="articleStatus!=3" align="center" label="预览图" width="80">
        <template slot-scope="scope">
          <el-popover
            placement="left"
            trigger="hover">
            <img :src="scope.row.imageUrl" width="600">
            <img slot="reference" :src="scope.row.imageUrl" :alt="scope.row.image" width="60">
          </el-popover>
        </template>
      </el-table-column>
...
<script>
import { getPageArticle, modifyArticle, deleteArticle } from '@/api/article'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getPageArticle(this.listQuery).then(response => {
    ...
    sortChange(data) {
      if (data.order === 'ascending') {
        this.listQuery.sort = data.prop + ",asc"
      } else {
        this.listQuery.sort = data.prop + ",desc"
      }
      this.handleFilter()
    },
    tagType(index) {
