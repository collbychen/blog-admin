<template>
  <div class="app-container">
    ...
    <pagination v-show="total>0" :total="this.total" :page.sync="listQuery.number" :limit.sync="listQuery.number" @pagination="getList()" />
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getPageComment(this.listQuery).then(response => {
    ...
    sortChange(data) {
      if (data.order === 'ascending') {
        this.listQuery.sort = data.prop.replace(/([A-Z])/g, '_$1').toLowerCase() + ",asc"
      } else {
        this.listQuery.sort = data.prop.replace(/([A-Z])/g, '_$1').toLowerCase() + ",desc"
      }
      this.handleFilter()
    },
